...
def listDetection(img, link=None):
    ...
    detected_circles = cv2.HoughCircles(gray_blurred, cv2.HOUGH_GRADIENT, 2, 80, param1=60, param2=160, minRadius=20, maxRadius=150)
        ...
        for pt in detected_circles[0, :]:
            ...
            if x >= 0 and y >= 0 and r >= 0:
                ...
                if x+w < imgr.shape[0] and x+h < imgr.shape[1]:
                    ...
                    b, g, r = cv2.split(imgtmp)  # get b, g, r
                    imgtmp = cv2.merge([r, g, b])
...
imgr = cv2.imread('image_test/download.jpeg', cv2.IMREAD_COLOR)
...
for i in range(len(sign)):
    ...
    b = bounding[i]
    imgr = cv2.rectangle(imgr, (b[1], b[0]), (b[1] + b[2], b[0] + b[3]), (36, 255, 12), 1)
    cv2.putText(imgr, classNames[sign[i]], (b[1], b[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)
